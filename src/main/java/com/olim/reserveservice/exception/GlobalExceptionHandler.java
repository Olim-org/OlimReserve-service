...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<Object> handleDuplicateEmailException(DuplicateException ex) {
        // You can create a custom response body if needed
        log.error("handleDuplicateEmailException", ex);
        final ErrorResponse response = ErrorResponse.of(ErrorCode.CONFLICT_ERROR, ex.getMessage());
    ...
    public ResponseEntity<Object> handleAuthenticationFailException(AuthenticationFailException ex) {
        // It's a good practice to send generic error messages in such cases
        final ErrorResponse response = ErrorResponse.of(ErrorCode.UNAUTHORIZED_ERROR, ex.getMessage());

        return new ResponseEntity<>(response, HttpStatus.UNAUTHORIZED); // 401 Unauthorized
    }
    @ExceptionHandler(VerifyEmailFailException.class)
    public ResponseEntity<Object> handleVerifyEmailFailException(VerifyEmailFailException ex) {
        // It's a good practice to send generic error messages in such cases
    ...
    public ResponseEntity<Object> handleDataNotFoundException(DataNotFoundException ex) {
        ...
        return new ResponseEntity<>(response, HttpStatus.NOT_FOUND); // 401 Unauthorized
    }
    @ExceptionHandler(PermissionFailException.class)
    public ResponseEntity<Object> handlePermissionFailException(PermissionFailException ex) {
        // It's a good practice to send generic error messages in such cases
